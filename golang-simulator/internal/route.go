...
func (rs *RouteService) CreateRoute(route Route) (Route, error) {
	...
	_, err := rs.mongo.Database("gosimulator").Collection("routes").UpdateOne(nil, filter, update, opts)
...
func (rs *RouteService) GetRoute(id string) (Route, error) {
	...
	err := rs.mongo.Database("gosimulator").Collection("routes").FindOne(nil, filter).Decode(&route)
