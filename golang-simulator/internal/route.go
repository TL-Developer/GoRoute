...
func (routeService *RouteService) CreateRoute(route *Route) (*Route, error) {
	...
	_, err := routeService.mongo.Database("gosimulator").Collection("routes").UpdateOne(nil, filter, update, opts)
...
func (routeService *RouteService) GetRoute(id string) (Route, error) {
	...
	err := routeService.mongo.Database("gosimulator").Collection("routes").FindOne(nil, filter).Decode(&route)
