...
export async function searchDirections(source: string, destination: string) {
  const [sourceResponse, destinationResponse] = await Promise.all([
    fetch(`${process.env.NEXT_PUBLIC_BASE_API_V1}/places?text=${source}`, {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BASE_API_V1}/places?text=${destination}`, {
      cache: "force-cache", //default
      next: {
        revalidate: 10, // 1 dia
        // revalidate: 1 * 60 * 60 * 24, // 1 dia
  ...
  const placeSourceId = sourceData.output.candidates[0].place_id;
  const placeDestinationId = destinationData.output.candidates[0].place_id;

  const directionsResponse = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_API_V1}/directions?originId=${placeSourceId}&destinationid=${placeDestinationId}`,
    {
      cache: "force-cache", //default
      next: {
        revalidate: 10, // 1 dia
  ...
  return {
    directionsData: directionsData.output,
    placeSourceId,
    placeDestinationId,
  };
}
    ...
    <div>
      <div className="flex flex-1 w-full h-full">
        <div className="w-1/3 p-4 h-full">
          <h4 className="text-3xl text-contrast mb-8">Nova rota</h4>
